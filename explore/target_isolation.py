"""Host-enforced public-egress boundary for RSI TARGET evolution.
...
log = logging.getLogger("rsiagent.practice.egress")

ALLOWED_STREAMVIEW_HOST = "streamview.web.hku.icu"
POLICY_VERSION = "target-egress-seal-v1"
GUEST_BRIDGE = "dockerbridge"
NAT_CHAIN = "RSI_TARGET_NAT"
FORWARD_CHAIN = "RSI_TARGET_FWD"
INPUT_CHAIN = "RSI_TARGET_IN"
OUTPUT_CHAIN = "RSI_TARGET_OUT"
L2_CHAIN = "RSI_TARGET_L2"
MAX_PREFACE_BYTES = 65_536


class RSIEgressError(RuntimeError):
...
def _new_gateway_identity(hostname: str) -> GatewayIdentity:
    ...
    ca_name = x509.Name([
        x509.NameAttribute(NameOID.COMMON_NAME, "RSI TARGET ephemeral egress CA"),
    ])
    ca_cert = (
...
def _server_ssl_context(
    ...
    with tempfile.TemporaryDirectory(prefix="target-egress-cert-") as temp:
...
class FilteringTcpGateway:
    ...
    def ports(self) -> GatewayPorts:
        if self._ports is None:
            raise RSIEgressError("filtering gateway has not started")
        return self._ports
    ...
    def start(self) -> GatewayPorts:
        ...
        for listener, protocol in zip(listeners, ("http", "https")):
            thread = threading.Thread(
                ...
                name=f"target-egress-{protocol}", daemon=True)
            thread.start()
    ...
    def _accept_loop(self, listener: socket.socket, protocol: str) -> None:
        while not self._stop.is_set():
            ...
            thread = threading.Thread(
                ...
                name=f"target-egress-{protocol}-connection")
            thread.start()
    ...
    def _handle_guarded(self, client: socket.socket, protocol: str) -> None:
        try:
            ...
            log.debug("RSI egress gateway rejected %s connection: %s",
                      protocol, exc)
            if protocol == "http":
                try:
                    client.sendall(
...
def _exec_result(result: Any) -> tuple[int, str]:
    if hasattr(result, "exit_code"):
        ...
        raise RSIEgressError("outer-container exec returned an unknown result")
...
def _validated_gateway_endpoint(gateway_ip: str, ports: GatewayPorts) -> None:
    ...
    if address.version != 4 or address.is_unspecified or address.is_multicast:
        raise RSIEgressError("host gateway is not a usable IPv4 address")
    for port in (ports.http, ports.https):
        if not isinstance(port, int) or not 1 <= port <= 65_535:
            raise RSIEgressError("host gateway port is invalid")
...
def _firewall_script(
    ...
    if mode not in {"target", "null"}:
        raise RSIEgressError("egress mode must be target or null")
    if mode == "target":
        if gateway_ip is None or ports is None:
            raise RSIEgressError("target egress requires a host gateway")
...
set +e
practice_ip6_probe=$(ip6tables -w -S 2>&1)
practice_ip6_rc=$?
set -e
if test "$practice_ip6_rc" -eq 0; then
  practice_ipv6_mode=ip6tables-plus-ebtables
elif test "$practice_ip6_rc" -eq 3 && \
     printf '%s' "$practice_ip6_probe" | grep -Fq 'Table does not exist'; then
  practice_ipv6_mode=ebtables-authoritative-no-outer-ipv6
  test -z "$(ip -6 -o addr show scope global)"
  test -z "$(ip -6 route show default)"
else
  printf 'unexpected ip6tables capability failure rc=%s: %s\n' \
    "$practice_ip6_rc" "$practice_ip6_probe" >&2
  exit 73
fi
...
if test "$practice_ipv6_mode" = ip6tables-plus-ebtables; then
...
{target_checks}
echo RSI_EGRESS_IPV6_MODE=$practice_ipv6_mode
echo RSI_EGRESS_SEAL_OK={mode}
"""
...
def install_outer_firewall_receipt(
    ...
    marker = f"RSI_EGRESS_SEAL_OK={mode}"
    ipv6_markers = [
        line.split("=", 1)[1] for line in output.splitlines()
        if line.startswith("RSI_EGRESS_IPV6_MODE=")]
    accepted_ipv6 = {
        "ip6tables-plus-ebtables",
        "ebtables-authoritative-no-outer-ipv6",
    }
...
def _outer_container(environment: Any) -> Any:
    ...
    if container is None or not callable(getattr(container, "exec_run", None)):
        raise RSIEgressError(
            "RSI TARGET egress seal requires the live Docker provider container")
    try:
        container.reload()
    except Exception as exc:  # noqa: BLE001 - Docker SDK boundary
        raise RSIEgressError("could not inspect the outer Docker container") from exc
    if getattr(container, "status", "") != "running":
        raise RSIEgressError("outer Docker container is not running")
    return container
...
class TargetIsolationSeal:
    ...
    def _install_guest_target_identity(
        ...
        if not password:
            raise RSIEgressError("guest elevation password is unavailable")
...
printf '%s\n' "$pw" | sudo -S -p '' sh -ceu '
  printf %s {ca_b64} | base64 -d > \
    /usr/local/share/ca-certificates/target-egress.crt
  chmod 0644 /usr/local/share/ca-certificates/target-egress.crt
  update-ca-certificates >/dev/null
  sed -i "/[[:space:]]# RSI_TARGET_EGRESS$/d" /etc/hosts
  printf "%s\\t%s\\t# RSI_TARGET_EGRESS\\n" \
    "{gateway_ip}" "{self.allowed_host}" >> /etc/hosts
'
unset pw
test "$(getent ahostsv4 '{self.allowed_host}' | awk 'NR==1 {{print $1}}')" = \
  '{gateway_ip}'
echo RSI_GUEST_RELAY_IDENTITY_OK=1
"""
        output = vm.run_command(command, timeout=180, cap=4096) or ""
        if "RSI_GUEST_RELAY_IDENTITY_OK=1" not in output:
            raise RSIEgressError(
    ...
    def install(self, vm: Any, *, mode: str) -> SealInstallation:
        ...
        if not container_id:
            raise RSIEgressError("outer Docker container lacks an identity")
        with self._lock:
            if mode == "target":
    ...
    def probe(
        ...
        if installation.mode not in {"target", "null"}:
            raise RSIEgressError("cannot probe an unknown egress mode")
...
def blocked(name, family, address, port, udp=False):
    ...
    try:
        ...
        print(f"RSI_PROBE_{name}=BLOCKED")
    else:
        print(f"RSI_PROBE_{name}=OPEN")
    finally:
        sock.close()
...
if curl --noproxy '*' -fsS --connect-timeout 8 --max-time 25 \
  ...
  echo RSI_PROBE_EXACT=PASS
else
  echo RSI_PROBE_EXACT=BLOCKED
fi
if curl --noproxy '*' -kfsS --connect-timeout 5 --max-time 10 \
  ...
  echo RSI_PROBE_WRONG_SNI=OPEN
else
  echo RSI_PROBE_WRONG_SNI=BLOCKED
fi
if curl --noproxy '*' -kfsS --connect-timeout 5 --max-time 10 \
  'https://{pinned_ip}/' >/dev/null 2>&1; then
  echo RSI_PROBE_DIRECT_IP=OPEN
else
  echo RSI_PROBE_DIRECT_IP=BLOCKED
fi
printf %s {encoded_probe} | base64 -d | python3 -
echo RSI_PROBE_DONE=1
"""
        ...
        for line in output.splitlines():
            if not line.startswith("RSI_PROBE_") or "=" not in line:
                continue
            name, value = line.removeprefix("RSI_PROBE_").split("=", 1)
            if name in observed:
                raise RSIEgressError("egress probe emitted a duplicate marker")
            observed[name] = value
