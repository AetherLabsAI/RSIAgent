...
def test_target_firewall_redirects_only_http_https_then_drops_everything_else():
    ...
    assert "ebtables -A E15_V12_L2 -p IPv6 -j DROP" in script
    assert "qemu-system-x86_64" in script
    assert "/sys/class/net/qemu/master" in script
    assert "iptables -w -I OUTPUT 1 -j E15_V12_OUT" in script
...
def test_null_firewall_has_no_gateway_allow_and_still_installs_drop_chains():
    ...
    assert f"-A E15_V12_OUT -j DROP" in script
    assert "E15_EGRESS_SEAL_OK=null" in script
...
class _Container:
    ...
    def exec_run(self, argv, **kwargs):
        ...
        mode = "target" if "E15_EGRESS_SEAL_OK=target" in argv[-1] else "null"
        output = (
            f"E15_EGRESS_IPV6_MODE={self.ipv6_mode}\n"
            f"E15_EGRESS_SEAL_OK={mode}\n").encode() if self.marker else b""
...
def test_outer_firewall_install_fails_closed_on_exec_or_marker_failure():
    with pytest.raises(E15EgressError):
...
def test_real_image_missing_ip6_filter_has_narrow_ebtables_fallback():
    script = _firewall_script(mode="null")
    assert "e15_ip6_rc\" -eq 3" in script
...
def test_live_probe_seam_requires_positive_exact_and_all_negative_markers():
    output = "\n".join([
        "E15_PROBE_EXACT=PASS",
        "E15_PROBE_WRONG_SNI=BLOCKED",
        "E15_PROBE_DIRECT_IP=BLOCKED",
        "E15_PROBE_DNS=BLOCKED",
        "E15_PROBE_OUTER_GATEWAY=BLOCKED",
        "E15_PROBE_NON_HTTP=BLOCKED",
        "E15_PROBE_IPV6=BLOCKED",
        "E15_PROBE_DONE=1",
    ])
    seal = object.__new__(V12EgressSeal)
...
def test_precreated_chrome_identity_is_reused_and_started_after_real_rebind():
    seal = V12EgressSeal(gateway_factory=_FakeGateway)
